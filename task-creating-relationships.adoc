...
** https://docs.netapp.com/us-en/storage-management-azure-netapp-files/task-quick-start.html["Configurando e descobrindo o Azure NetApp Files"^]
...
** https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-adding-systems.html["Adicionando sistemas Cloud Volumes ONTAP existentes"^]
** https://docs.netapp.com/us-en/storage-management-ontap-onprem/task-discovering-ontap.html["Descobrindo clusters ONTAP"^]
...
+
Agendar:: Escolha uma programação recorrente para sincronizações futuras ou desative a programação de sincronização. Você pode programar um relacionamento para sincronizar dados a cada 1 minuto. As programações também podem ser definidas com frequência horária e diária.
+
--
...
A função Copiar e Sincronizar inicia a sincronização de dados entre a origem e o destino. Estatísticas sobre quanto tempo a sincronização levou, se foi interrompida e quantos arquivos foram copiados, verificados ou excluídos estão disponíveis. Em seguida, você pode gerenciar seu link:task-managing-relationships.html["sincronizar relacionamentos"], link:task-managing-data-brokers.html["gerencie seus corretores de dados"] ou link:task-managing-reports.html#creating-reports["crie relatórios para otimizar seu desempenho e configuração"].
...
image:screenshot-sync-data-sense.png["Uma captura de tela que mostra a página Integração de NetApp Data Classification que aparece após iniciar uma nova sincronização diretamente do NetApp Data Classification."]

https://docs.netapp.com/us-en/data-services-data-classification/concept-classification.html["Aprenda como iniciar um relacionamento de sincronização a partir da NetApp Data Classification"^].

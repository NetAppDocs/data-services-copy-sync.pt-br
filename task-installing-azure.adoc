...
----
As seguintes permissões são necessárias apenas se você planeja habilitar a link:task-creating-relationships.html["Configuração de sincronização contínua"] em um relacionamento de sincronização do Azure para outro storage de nuvem:
...
O escopo atribuível deve ser definido como escopo de assinatura e *não* como escopo de grupo de recursos se você planeja implementar Continuous Sync no Azure.
...
Ao implantar o agente de dados, você precisa escolher um método de autenticação para a máquina virtual: uma senha ou um par de chaves pública e privada SSH.

Para obter ajuda na criação de um par de chaves, consulte link:https://docs.microsoft.com/en-us/azure/virtual-machines/linux/mac-create-ssh-keys["Documentação do Azure: Criar e usar um par de chaves pública-privada SSH para VMs Linux no Azure"^].

...
* *Compatibilidade com Node.js*: v25.x
* *Sistema operacional*:
+
** CentOS 8.0 e 8.5
+
...
+
Um sistema Red Hat Enterprise Linux deve ser registrado no Red Hat Subscription Management. Se não estiver registrado, o sistema não poderá acessar os repositórios para atualizar o software de terceiros necessário durante a instalação.
...
. Na página *Definir relação de sincronização*, escolha uma origem e um destino e selecione *Continuar*.
+
...
. Insira um nome para o corretor de dados e selecione *Continuar*.
+
Após o carregamento da página de instruções, siga as instruções que incluem um link exclusivo para baixar o instalador.

. Na página de instruções:
+
.. Escolha habilitar o acesso à *AWS*, ao *Google Cloud* ou a ambos.
...
+
Chaves AWS:: Estas são as chaves para o usuário que você deve ter preparado<<Habilitar acesso à AWS,seguindo estes passos>> .  As chaves da AWS são armazenadas no data broker, que é executado na sua rede local ou na nuvem.  A NetApp não usa as chaves fora do data broker.
arquivo JSON:: Este é o arquivo JSON que contém uma chave de conta de serviço que você deve ter preparado<<Habilitar acesso ao Google Cloud,seguindo estes passos>> .
